resource "openstack_fw_firewall_v1" "cd-firewall" {
    name = "cd-firewall"
    policy_id = "${openstack_fw_policy_v1.cd-policy.id}"
}

resource "openstack_fw_policy_v1" "cd-policy" {
    name = "Open bar"
    rules = [
...
resource "openstack_fw_rule_v1" "in-allow-gocd" {
    name = "in-allow-gocd"
    protocol = "tcp"
    action = "allow"
